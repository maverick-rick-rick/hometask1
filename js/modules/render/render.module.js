...
import { notesQuantityByCategories, addNewNoteItem, dislocateNote, changeNoteItem} from "./../data/data_operations.module.js";
...
function fillNoteTable(table, arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        let row = DOMElements.newTableRow.tableRow(arr[i], {headline : false, buttons : true});
        row.dataset.index = i;
...
function fillQuantityTable(arr, el) {
    arr.forEach(element => {
        let row = DOMElements.newTableRow.tableRow(element, {headline : false, buttons : false});
        el.append(row);
    });
...
export function createNewTableItem(el) {
   ...
   let result =  DOMElements.newTableRow.tableRow(obj, {headline : false, buttons : true});
...
export function fillFormWithExistingNote(index, type){
        ...
        newNoteForm.querySelector('#create_note_category').value = currentNoteObj.category;
        newNoteForm.querySelector('#create_note_content').value = currentNoteObj.content;
